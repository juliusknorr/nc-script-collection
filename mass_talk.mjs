import { ensureAppIsEnabled, loopNTimes, runInParallel } from './helper.mjs'
import {
    createRandomUsers,
    createTalkRoom,
    addUserToTalkRoom,
...
const sendChatMessage = async (talkRoom, user) => {
    ...
    const mentionedUser = shouldMention
    ...
    const result = await fetch(
        `${config.nextcloudUrl}/ocs/v2.php/apps/spreed/api/v1/chat/${talkRoom}`,
        {
            method: 'POST',
            headers: {
    ...
    if (!result.ok) {
        console.error(
            `Error sending chat message to ${talkRoom} from ${user}: ${result.statusText}`
        )
    }
}
...
const uploadRandomFile = async (user) => {
    ...
    const uploadResponse = await fetch(
        `${config.nextcloudUrl}/remote.php/dav/files/${user}/Talk/${fileName}`,
        {
            method: 'PUT',
            headers: {
                Authorization: `Basic ${Buffer.from(`${user}:${user}`).toString('base64')}`,
    ...
    if (!uploadResponse.ok) {
        console.error(
            `Error uploading file for ${user}: ${uploadResponse.statusText}`
        )
        return null
    }
...
const shareFileToConversation = async (user, fileName, talkRoom) => {
    const shareResponse = await fetch(
        `${config.nextcloudUrl}/ocs/v2.php/apps/files_sharing/api/v1/shares`,
        {
            method: 'POST',
            headers: {
                Authorization: `Basic ${Buffer.from(`${user}:${user}`).toString('base64')}`,
    ...
    if (!shareResponse.ok) {
        console.error(
            `Error sharing file for ${user}: ${shareResponse.statusText}`
        )
        return false
    }
...
runInParallel(userList, async (user) => {
    // Create a Talk folder for each user via WebDAV
    const response = await fetch(
        `${config.nextcloudUrl}/remote.php/dav/files/${user}/Talk`,
        {
            method: 'MKCOL',
            headers: {
                Authorization: `Basic ${Buffer.from(`${user}:${user}`).toString('base64')}`,
    ...
    if (!response.ok) {
        console.error(
            `Error creating Talk folder for ${user}: ${response.statusText}`
        )
    } else {
    ...
    loopNTimes(100, async () => {
        const randomTalkRoom =
            talkRooms[Math.floor(Math.random() * talkRooms.length)]
